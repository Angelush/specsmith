---
title: Anthropic Just Gave You 3 Tools That Work While You're Gone.
type: source
video_id: 3e7gmNPr5Vo
url: https://www.youtube.com/watch?v=3e7gmNPr5Vo
playlists: [0]
concepts: ["claude-code-architecture"]
updated: 2026-05-12
---

# Anthropic Just Gave You 3 Tools That Work While You're Gone.

**Video:** [https://www.youtube.com/watch?v=3e7gmNPr5Vo](https://www.youtube.com/watch?v=3e7gmNPr5Vo) • **ID:** `3e7gmNPr5Vo` • **Playlists:** P0

## Concepts covered

- [[concepts/claude-code-architecture]] — Leveraging primitives like Dispatch, Scheduled Tasks, and Computer Use to create continuous, autonomous Claude agents.

## Entries extracted

| ID | Category | Concept | TL;DR |
|----|----------|---------|-------|
| TUL-005 | TOOL | [[concepts/claude-code-architecture]] | Combine Claude Dispatch, Scheduled Tasks, and Computer Use to build agents that operate continuously without human prompts. |
