---
title: Anthropic's Trojan Horse: How Claude Code Plus a Million Tokens Could Win the Workplace
type: source
video_id: 2qHxfwvIx-I
url: https://www.youtube.com/watch?v=2qHxfwvIx-I
playlists: [0, 3]
transcript: data/transcripts/2qHxfwvIx-I.txt
concepts: ['claude-code-architecture']
updated: 2026-05-12
---

# Anthropic's Trojan Horse: How Claude Code Plus a Million Tokens Could Win the Workplace

**Video:** [https://www.youtube.com/watch?v=2qHxfwvIx-I](https://www.youtube.com/watch?v=2qHxfwvIx-I) • **ID:** `2qHxfwvIx-I` • **Playlists:** P0, P3

## Concepts covered

- [[concepts/claude-code-architecture]] — Anthropic's strategy involves using code-agent dominance as a key entry point to secure general workplace agent ownership.

## Entries extracted

| ID | Category | Concept | TL;DR |
|----|----------|---------|-------|
| TRD-009 | TREND | [[concepts/claude-code-architecture]] | Consult when making platform bets; Anthropic uses code-agent dominance as entry to workplace agent ownership. |

## Transcript

[data/transcripts/2qHxfwvIx-I.txt](../data/transcripts/2qHxfwvIx-I.txt)