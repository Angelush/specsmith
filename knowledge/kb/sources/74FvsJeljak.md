---
title: 7 Prompting Strategies from Claude 4's System Prompt Leak
type: source
video_id: 74FvsJeljak
url: https://www.youtube.com/watch?v=74FvsJeljak
playlists: [1]
concepts: [advanced-prompting-techniques]
updated: 2026-05-12
---

# 7 Prompting Strategies from Claude 4's System Prompt Leak

**Video:** [https://www.youtube.com/watch?v=74FvsJeljak](https://www.youtube.com/watch?v=74FvsJeljak) • **ID:** `74FvsJeljak` • **Playlists:** P1

## Concepts covered

- [[concepts/advanced-prompting-techniques]] — System prompts function like operating system configuration files, defining session defaults, permissions, and identity.

## Entries extracted

| ID | Category | Concept | TL;DR |
|----|----------|---------|-------|
| PRM-012 | PROMPTING | [[concepts/advanced-prompting-techniques]] | System prompts are OS config files — they set behavioral defaults, permissions, and identity for the entire session. |
